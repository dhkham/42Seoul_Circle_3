...
/*   Updated: 2023/06/25 17:12:28 by dkham            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	args->num_of_philo = atoi(argv[1]); // 철학자의 수를
	args->time_to_die = atoi(argv[2]); // 철학자가 죽을 때까지의 시간
	args->time_to_eat = atoi(argv[3]); // 철학자가 식사하는 시간
	args->time_to_sleep = atoi(argv[4]); // 철학자가 잠을 자는 시간
	if (argc == 6)
		args->num_of_must_eat = atoi(argv[5]); // 철학자가 식사해야 하는 횟수
	else
